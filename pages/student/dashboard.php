...
@media(max-width:768px){.sidebar{transform:translateX(-100%)}.sidebar.open{transform:translateX(0)}.main{margin-left:0}.content{padding:1rem;overflow-x:hidden}.topbar{padding:.8rem 1rem}.stats-grid{grid-template-columns:repeat(2,1fr)}.data-table{font-size:.72rem}.data-table th,.data-table td{padding:.4rem .5rem}.topbar-title{font-size:.78rem}.stat-value{font-size:1.5rem}.code-inputs input{width:38px;height:48px;font-size:1.1rem}.code-inputs{gap:.4rem}.camera-wrap{max-width:100%}.attend-zone{padding:0}.tt-item{flex-direction:column;gap:.3rem}.tt-time{min-width:unset}#menu-btn{display:block}.hide-mobile{display:none!important}}
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="page-section" id="sec-courses">
      <div class="section-title">My <span>Courses</span><?php if($activeSem): ?><span style="font-size:.65rem;color:var(--muted);font-family:'DM Sans',sans-serif;letter-spacing:.1em;margin-left:.8rem"><?= htmlspecialchars($activeSem['name']) ?></span><?php endif; ?></div>
      <?php if(empty($enrolledCourses)): ?>
        <div class="card"><div class="card-body" style="color:var(--muted)">No courses enrolled this semester. Contact admin.</div></div>
      <?php else: ?>
        <div class="card"><div class="card-body" style="padding:0;overflow-x:auto">
          ...
          <?php foreach($enrolledCourses as $c):
            ...
            <tr>
              <td style="color:var(--gold);font-weight:600"><?= htmlspecialchars($c['code']) ?></td>
    ...
    <div class="page-section" id="sec-timetable">
      ...
      <?php endforeach; ?>
      <?php if(empty($enrolledCourseIds)): ?><p style="color:var(--muted);font-size:.83rem">No enrolled courses found.</p><?php endif; ?>
    </div>
    ...
    <div class="page-section" id="sec-history">
      ...
      <div class="card"><div class="card-body" style="padding:0;overflow-x:auto">
        ...
        <?php if(empty($hist)): ?><tr><td colspan="3" style="color:var(--muted)">No records yet.</td></tr>
        <?php else: foreach($hist as $r): ?>
          <tr><td><strong><?= htmlspecialchars($r['course_code']) ?></strong><br><span style="color:var(--muted);font-size:.72rem"><?= htmlspecialchars($r['course_name']) ?></span></td>
          <td><span class="pill pill-<?= $r['status']==='present'?'green':($r['status']==='late'?'gold':($r['status']==='pending'?'pending':'red')) ?>"><?= $r['status'] ?></span></td>
          <td style="color:var(--muted);font-size:.72rem;white-space:nowrap"><?= date('d M Y',strtotime($r['timestamp'])) ?><br><?= date('H:i',strtotime($r['timestamp'])) ?></td></tr>
        <?php endforeach; endif; ?>
        </tbody></table>
      </div></div>
    </div>
    ...
    <div class="page-section" id="sec-stats">
      ...
      <?php else: foreach($enrolledCourses as $c):
        ...
        <div class="course-bar">
          ...
          <div style="font-size:.68rem;color:var(--muted);margin-top:.2rem"><?= $c['attended'] ?> / <?= $c['total_sessions'] ?> sessions</div>
        </div>
      <?php endforeach; endif; ?>
    </div>
...
function captureSelfie(){const video=document.getElementById('video-preview');const canvas=document.getElementById('capture-canvas');canvas.width=video.videoWidth||320;canvas.height=video.videoHeight||240;canvas.getContext('2d').drawImage(video,0,0);if(cameraStep==='selfie'){capturedSelfie=canvas.toDataURL('image/jpeg',.8);document.getElementById('selfie-preview').src=capturedSelfie;document.getElementById('capture-btn').textContent='📸 Capture Classroom';document.getElementById('retake-btn').style.display='flex';document.getElementById('video-preview').style.display='block';stopCamera();cameraStep='classroom';navigator.mediaDevices.getUserMedia({video:{facingMode:{exact:'environment'}}}).then(s=>{stream=s;document.getElementById('video-preview').srcObject=s}).catch(()=>navigator.mediaDevices.getUserMedia({video:{facingMode:'user'}}).then(s=>{stream=s;document.getElementById('video-preview').srcObject=s}));}else{capturedClassroom=canvas.toDataURL('image/jpeg',.8);document.getElementById('selfie-preview').src=capturedClassroom;document.getElementById('capture-btn').style.display='none';document.getElementById('video-preview').style.display='none';document.getElementById('submit-btn').style.display='flex';document.getElementById('step-label').textContent='Step 3: Submit for Rep approval';stopCamera()}}
...
async function submitAttendance(){const btn=document.getElementById('submit-btn');const errEl=document.getElementById('submit-error');if(!capturedSelfie){errEl.textContent='Please take a selfie first.';errEl.style.display='block';return}btn.disabled=true;btn.textContent='Submitting...';errEl.style.display='none';try{const res=await fetch('../../api/mark_attendance.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_id:<?= $activeSession?$activeSession['id']:'null' ?>,selfie:capturedSelfie,classroom:capturedClassroom})});const data=await res.json();if(data.success){document.getElementById('step-selfie-section').innerHTML=`<div class="pending-card" style="margin-top:1rem"><div class="pending-icon">⏳</div><div class="pending-title">Submitted!</div><div class="pending-sub">Your selfie is pending approval from the Course Rep.</div></div>`;}else{errEl.textContent=data.message||'Submission failed. Try again.';errEl.style.display='block';btn.disabled=false;btn.textContent='Submit →'}}catch(e){errEl.textContent='Connection error. Try again.';errEl.style.display='block';btn.disabled=false;btn.textContent='Submit →'}}
