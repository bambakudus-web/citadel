...
if (!$semId) {
    $q = $pdo->prepare("SELECT id FROM semesters WHERE is_active=1 AND institution_id=? LIMIT 1");
    $q->execute([$inst_id]);
    $row   = $q->fetch();
    $semId = $row['id'] ?? null;
}
    ...
    switch ($type) {
        ...
        case 'trend':
            $stmt = $pdo->prepare("
                ...
                LIMIT 10
            ");
            $stmt->execute([$inst_id, $semId, $semId]);
        ...
        case 'courses':
            $stmt = $pdo->prepare("
                ...
                WHERE c.semester_id = ? AND sem.institution_id = ?
                GROUP BY c.id
                ORDER BY c.code ASC
            ");
            $stmt->execute([$semId, $inst_id]);
            $rows   = $stmt->fetchAll();
            $labels = [];
            $rates  = [];
            foreach ($rows as $r) {
